...
// CLEAN
//
// TODO: USE_SIMD, USE_WIDEXOR, XORSIMD & Pattern detector logic
module DSP48E2 #(
    ...
    parameter PREADDINSEL = "A",                // Selects the input to be added with D in the preadder
    parameter AMULTSEL = "A",                   // Selects the input to the 27-bit A input of the multiplier (former USE_DPORT)
    parameter BMULTSEL = "B",                   // Selects the input to the 18-bit B input of the multiplier
    ...
    parameter [47:0] RND = 48'h0                // This 48-bit value is used as the Rounding Constant into the WMUX
) (
	...
	input [8:0]    OPMODE,                      // 9-bit input: Operation mode input   *** EXTENDEN IN US ***
	...
	input [26:0]   D,                           // 27-bit input: D data input          *** EXTENDEN IN US ***
...
wire       INMODE_1A   = (PREADDINSEL == "A") ? INMODE_IN[1] : 1'b0;
wire       INMODE_1B   = (PREADDINSEL != "A") ? INMODE_IN[1] : 1'b0;
...
wire [26:0] A_OUT      = (INMODE_IN[1]) ? 27'h0 : (~INMODE_IN[0] ? A_IN_ST2[26:0] : A_IN_ST1[26:0]);
wire [26:0] A2A1       = (!INMODE_1A) ? A_OUT : 27'h0;
wire [26:0] PREADD_AB  = (PREADDINSEL == "A") ? A2A1 : {{9{B2B1[17]}}, B2B1};

wire [26:0] D_IN_ST0   = (DREG == 0) ?  D : D_reg;
wire [26:0] D_IN_ST1   = (~INMODE_IN[2]) ? 30'h0 : D_IN_ST0;
//wire [26:0] AD_SUM     = INMODE_IN[3] ?  D_IN_ST1 - A_OUT : D_IN_ST1 + A_OUT;
wire [26:0] AD_SUM     = INMODE_IN[3] ?  D_IN_ST1 - PREADD_AB : D_IN_ST1 + PREADD_AB;

wire [26:0] AD_SUM_ST0 = (ADREG) ? AD_reg : AD_SUM;
wire [26:0] A_MULT     = (AMULTSEL == "A") ? A2A1 : AD_SUM_ST0;
...
assign B2B1            = (!INMODE_1B) ? (INMODE_IN[4] ? B_IN_ST1 : B_IN_ST2) : 18'h0;
wire [17:0] B_MULT     = (BMULTSEL == "B") ? B2B1 : AD_SUM_ST0[17:0];

wire        AXNORB     = (A_MULT[26] == B_MULT[17]);

// it's partial product, mot MULT !!!!
wire [44:0] MULT       = $signed(A_MULT) * $signed(B_MULT);
wire [44:0] MULT_ST0   = MREG ? M_reg : MULT;
wire        AXNORB_ST0 = MREG ? AXNORB_reg : AXNORB;
wire [44:0] C_ST0      = CREG ? C_reg : C;

wire        CARRYIN_IN = CARRYINREG ? CARRYIN_reg : CARRYIN;
// X Y Z 
wire [3:0]  ALUMODE_IN = (ALUMODEREG) ? ALUMODE_reg : ALUMODE;
wire [8:0]  OPMODE_IN  = (OPMODEREG)  ? OPMODE_reg  : OPMODE;
wire [2:0]  CARRYINSEL_IN = (CARRYINSELREG) ? CARRYINSEL_reg : CARRYINSEL;

wire [47:0] MUL_EXT    = {{3{MULT_ST0[44]}}, MULT_ST0 };
...
wire [47:0] W_MUX =
	(OPMODE_IN[8:7] == 0) ? 48'h0 :
	(OPMODE_IN[8:7] == 1) ? P_reg :
	(OPMODE_IN[8:7] == 2) ? RND : C_ST0;
...
wire [48:0] ALU_OUT =
	(ALUMODE_IN == 0) ? Z_MUX + W_MUX + X_MUX + Y_MUX + CIN :
	(ALUMODE_IN == 1) ? Z_MUX - (W_MUX + X_MUX + Y_MUX + CIN) :
	(ALUMODE_IN == 2) ? ~Z_MUX + W_MUX + X_MUX + Y_MUX + CIN :
	(ALUMODE_IN == 3) ? ~(Z_MUX + W_MUX + X_MUX + Y_MUX + CIN) :
